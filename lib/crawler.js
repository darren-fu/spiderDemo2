...
process.on('message', function (m) {
    fs.readFile(path.normalize(__dirname + '/../config/' + m), function (err, data) {
        ...
        console.log(config.isPagination);
    ...
    async.eachSeries(config.selector, function (item, callback) {
        ...
        if (config.selector.length == 1) {
            ...
            if (config.type) {
                if (that[config.type]) {
                    console.log('123456');
                    that[config.type](rooturl(config.from));
                ...
                if (config.isPagination) {
                    ...
                    async.whilst(function () {
                        ...
                        if (err) {
                            ...
                            urlLevels[0].forEach(function (item) {
                                that.log('收集到的url---->' + item, 'green');
                            });
            ...
            if (status) {
                ...
                if (len > 0) {
                    $$.each(function () {
                        ...
                        console.log('last.attr-->' + last.attr + '--url-->' +url);
...
Crawler.prototype.dlImage = function (list, callback) {
    ...
    list = list.filter(function (v) {
        return v.url ? true : false;
    });

    var count = list.length;
    that.log('准备下载到本地中...' + count + ' pics');
    if (count < 1) {
        callback();
        return;
    }
